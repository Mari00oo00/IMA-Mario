...
**代价函数（损失函数）：**$$J(\theta_0,\theta_1,\cdots,\theta_n)=J(\theta)=\frac{1}{2m} \sum\limits_{i=1}^m(h_\theta(x^{(i)})-y^{(i)})^2=\frac{1}{2m}(X\theta-Y)^2$$（这里乘1/2是为了下一步求偏导数时式子更美观）
...
规定$$\theta$$一开始全为0，且为列向量；学习率为2，次数50000次
