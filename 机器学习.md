...
1. 通过测试来评估学习器的泛化误差：
   ...
   * 使用测试集，但测试集与训练集的关系应该尽可能小。
   ...
   1. **留出法：**
      * 将数据集划分成两个互斥的集合
      * 注意：应至少保持样本的类别比例相近；单次使用留出法得到的估计结果往往不可靠，因此，在使用留出法时一般采用若干次随机划分、重复进行实验评估后取平均值作为留出法的评估结果
   2. **交叉验证法（$$k$$折交叉验证）：**
...
[ID3决策树中连续值的处理](https://blog.csdn.net/Leafage_M/article/details/80137305?ops_request_misc=%7B%22request%5Fid%22%3A%22164474507316780261959620%22%2C%22scm%22%3A%2220140713.130102334..%22%7D&request_id=164474507316780261959620&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduend~default-2-80137305.first_rank_v2_pc_rank_v29&utm_term=决策树连续值处理&spm=1018.2226.3001.4187)
...
def classify_category(feature, i):
    ...
    number_feature_category = data_feature_name_i.iloc[:, -1].value_counts()  # Dik的样本数列表
    return number_feature_category, data_feature_name_i
```

**计算条件熵的函数**

[python中统计列表各个元素的个数](https://blog.csdn.net/weixin_30602505/article/details/98010252?ops_request_misc=&request_id=&biz_id=102&utm_term=统计列表不同元素个数&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduweb~default-2-98010252.first_rank_v2_pc_rank_v29&spm=1018.2226.3001.4187)

细节：由于`value_counts()`与`groupby()`和`sort_values()`的排序方法不同，且这里的函数大多用后两者，所以这里求“特征各种类所含的样本数”，即$$|D_i|$$的时候用自己写的方法。
...
def get_conditional_entropy(feature, data_set):
    ...
    category_number_feature = []  # 特征各种类所含的样本数
    values = []  # 用于查重
    for value in list(feature[0]):
        if value not in values:
            values.append(value)
            category_number_feature.append(list(feature[0]).count(value))
    number_feature = len(list(feature.groupby([0]).groups.keys()))  # 特征的取值种类数目
    for i in range(number_feature):
        number_feature_category, data_i = classify_category(feature, i)  # Dik的样本数列表和Di(DataFrame格式)
...
细节：在传列表的时候要先复制，传的是复制的列表。因为列表与C语言的数组一样，传的是地址，是会对原数组进行修改的。
...
def tree_ID3(data_set, featureName):
    ...
    :return:
    """
    category_list = list(data_set.iloc[:, -1])  # 提取类别
    ...
    num_columns = data_set.shape[1]  # 获取列数
    number_category = len(data_set.iloc[:, -1].value_counts())  # 类的种类数目
    information_gain = []  # 信息增益列表
    for i in range(data_set.shape[1]-1):  # 计算每个特征的信息增益
        base_entropy = get_entropy(number_category, data_set.iloc[:, -1].value_counts(), data_set)  # 熵

        feature_i = data_set[[i, num_columns-1]]  # 提取特征
        feature_i.sort_values(by=[i]).reset_index(drop=True)  # 重新排序
        feature_i.columns = range(2)  # 重新设置列索引

        conditional_entropy = get_conditional_entropy(feature_i, data_set)  # 条件熵
    ...
    for value in kind_target_feature:  # 开始划分数据集并进行递归
        ...
        substitute_featureName = featureName[:]  # 复制操作
...
`{'纹理': {'模糊': '否', '清晰': {'脐部': {'凹陷': '是', '平坦': '否', '稍凹': {'色泽': {'青绿': '是', '乌黑': {'触感': {'硬滑': '是', '软粘': '否'}}}}}}, '稍糊': {'触感': {'硬滑': '否', '软粘': '是'}}}}`

**总结**

最花时间的反而是代码的实现部分，困难主要体现在：不知道如何构造函数、要构造什么函数、函数的接口怎么才能设置合理、如何划分数据集，以及决策树怎么呈现。解决的过程：通过看博客和拆分数学公式（最重要的是找相同点）就可以了解整个算法的构造，于是就自己动手写函数。在这个过程中最难受的是划分数据集这一步，这主要是由于自己对数据分析函数的掌握太生疏了，我于是不断在纸上画图想思路，然后上博客查找方法，终于是熟练了不少。函数的接口设置应该提前想好（知道大概的构造后就可以规划了），变量的命名问题现在还不能完全解决（意思相近的变量有点多）。总之，有耐心就行了🤡

#### C4.5算法

C4.5算法的基本过程与ID3算法一致，唯一的不同是采用信息增益比来选择最佳特征
...
```python
def get_max_information_gain_ratio(data_information_gain_ratio):
    """
    :param data_information_gain_ratio: 包含多个信息增益比的列表
    :return: 最大信息增益比及其所对应的特征所在的列的编号
    """
    max_information_gain = 0
    number_best_feature = 0
    for i in range(len(data_information_gain_ratio)):
        if data_information_gain_ratio[i] > max_information_gain:
            max_information_gain = data_information_gain_ratio[i]
            number_best_feature = i
    return number_best_feature
```

**构建C4.5决策树**

```python
def tree_C45(data_set, featureName):
    """
    :param data_set: 训练集
    :param featureName: 当前所含特征名的列表
    :return:
    ...
    num_columns = data_set.shape[1]  # 获取列数
    number_category = len(data_set.iloc[:, -1].value_counts())  # 类的种类数目
    information_gain_ratio = []  # 信息增益比列表
    for i in range(data_set.shape[1]-1):  # 计算每个特征的信息增益
        base_entropy = get_entropy(number_category, data_set.iloc[:, -1].value_counts(), data_set)  # 熵
        ...
        conditional_entropy = get_conditional_entropy(feature_i, data_set)  # 条件熵
        information_gain = base_entropy - conditional_entropy  # 信息增益
        information_gain_ratio.append(get_information_gain_ratio(feature_i, data_set, information_gain))
    num_best_feature = get_max_information_gain_ratio(information_gain_ratio)  # 最大信息增益所对应的特征所在的列的编号
    name_best_feature = featureName[num_best_feature]  # 最大信息增益比所对应的特征的名字
    del featureName[num_best_feature]
    ...
    for value in kind_target_feature:  # 开始划分数据集并进行递归
        data_set_new = classify_data(data_set, num_best_feature, value)
        substitute_featureName = featureName[:]  # 复制操作
        tree[name_best_feature][value] = tree_C45(data_set_new, substitute_featureName)
    return tree
```

**结果**

`{'纹理': {'清晰': {'敲声': {'沉闷': '是', '浊响': {'触感': {'软粘': {'色泽': {'青绿': '是', '乌黑': '否'}}, '硬滑': '是'}}, '清脆': '否'}}, '模糊': '否', '稍糊': {'触感': {'软粘': '是', '硬滑': '否'}}}}`

#### 剪枝

[决策树python源码实现（含预剪枝和后剪枝）](https://blog.csdn.net/ylhlly/article/details/93213633?ops_request_misc=%7B%22request%5Fid%22%3A%22164500366816780274156295%22%2C%22scm%22%3A%2220140713.130102334..%22%7D&request_id=164500366816780274156295&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduend~default-3-93213633.first_rank_v2_pc_rank_v29&utm_term=后剪枝python&spm=1018.2226.3001.4187)

决策树生成算法容易构造出过于复杂的决策树，从而导致过拟合，因此需要简化决策树。在决策树学习中将已生成的树进行简化的过程称为剪枝。

##### 预剪枝

在决策树**生成过程中**，对每个结点在**划分前**先进行估计，若当前结点的划分不能带来决策树**泛化性能**提升，则停止划分并将当前结点标记为叶结点。

*（用“贪心”的思想，比较划分前后的泛化性能）*

性能度量：精度，即分类正确的样本数/总样本数

优缺：降低了资源的开销，但可能会有欠拟合的风险（有可能在之后的划分所得的精度比剪枝时的更高）。
...
```python
def get_correct_number(data_set_train, data_set_test):
    """
    :param data_set_train: 训练集
    :param data_set_test: 验证集
    :return: 分类正确的数目
    """
    assume = majorityCnt(data_set_train.iloc[:, -1])  # 假定的值为含有样本数最多的类
    list_test_category = data_set_test.iloc[:, -1]  # 验证集的类别列表
    correct = 0  # 分类正确的数目
    for value in list_test_category:
        if assume == value:
            correct += 1
    return correct
```
...
```python
def pre_pruning(data_set_train, data_set_test, kind_feature, number_best_feature):  # 预剪枝的判断
    """
    :param data_set_train: 训练集
    :param data_set_test: 验证集
    :param kind_feature: 特征的不同取值（不含重复值）
    :param number_best_feature:最大信息增益所对应的特征所在的列的编号
    :return: 布尔类型，0代表禁止划分，1代表可以划分
    """
    pre_accuracy = get_correct_number(data_set_train, data_set_test)/len(data_set_test.iloc[:, -1])  # 划分前的精度
    number_correct = 0
    for value in kind_feature:
        train_new = classify_data(data_set_train, number_best_feature, value)  # 划分训练集
        test_new = classify_data(data_set_test, number_best_feature, value)  # 划分验证集
        number_correct += get_correct_number(train_new, test_new)  # 得到当前分类正确的样本数
    later_accuracy = number_correct/len(data_set_test.iloc[:, -1])  # 划分之后的精度
    if pre_accuracy >= later_accuracy:
        return 0
    else:
        return 1
```
...
步骤：判断特殊情况--计算多组信息增益并选择最大值对应的特征--预剪枝的判断--若继续划分则进入函数的递归，否则返回所含样本数最多的类

```python
def tree_ID3(data_set_train, data_set_test, featureName):
    """
    :param data_set_train: 训练集
    :param data_set_test:  验证集
    :param featureName: 当前所含特征名的列表
    ...
    category_list = list(data_set_train.iloc[:, -1])  # 提取类别
    if category_list.count(category_list[0]) == len(category_list):
        return category_list[0]  # 训练集中所有实例都属于同一类
    if len(featureName) == 1:  # 只有一个特征时返回训练集中实例数最大的类的值
        return majorityCnt(category_list)

    num_columns = data_set_train.shape[1]  # 获取列数
    number_category = len(data_set_train.iloc[:, -1].value_counts())  # 类的种类数目
    information_gain = []  # 信息增益列表
    for i in range(data_set_train.shape[1]-1):  # 计算每个特征的信息增益
        base_entropy = get_entropy(number_category, data_set_train.iloc[:, -1].value_counts(), data_set_train)  # 熵

        feature_i = data_set_train[[i, num_columns-1]]  # 提取特征
        feature_i.sort_values(by=[i]).reset_index(drop=True)  # 重新排序
        feature_i.columns = range(2)  # 重新设置列索引

        conditional_entropy = get_conditional_entropy(feature_i, data_set_train)  # 条件熵
        information_gain.append(base_entropy - conditional_entropy)
    num_best_feature = get_max_information_gain(information_gain)  # 最大信息增益所对应的特征所在的列的编号
    name_best_feature = featureName[num_best_feature]  # 最大信息增益所对应的特征的名字
    del featureName[num_best_feature]

    target_feature = data_set_train[num_best_feature]  # 提取目标特征序列
    kind_target_feature = set(target_feature)  # 把目标特征序列的不同取值提取出来，形成集合

    judge = pre_pruning(data_set_train, data_set_test, kind_target_feature, num_best_feature)  # 预剪枝的判断
    if judge == 0:  # 不需要划分，返回训练集中所含样本数最多的类
        return majorityCnt(data_set_train.iloc[:, -1])

    tree = {name_best_feature: {}}  # 字典的嵌套使用
    for value in kind_target_feature:  # 开始划分数据集并进行递归
        data_set_train_new = classify_data(data_set_train, num_best_feature, value)  # 划分数据集
        substitute_featureName = featureName[:]  # 复制操作
        tree[name_best_feature][value] = tree_ID3(data_set_train_new, data_set_test, substitute_featureName)
    return tree
```
...
先从训练集生成一颗完整的决策树，然后**自底向上**地对非叶结点进行考察，若将该结点对应的子树替换为叶结点能够带来决策树泛化性能的提升，则将该子树替换为叶结点。

特点及优缺：形成了决策树后再进行剪枝，从最细分的非叶结点开始，删除其叶结点，并将其标记为叶结点，计算此时的精度，与没剪之前对比。与预剪枝相比，优点是欠拟合的风险很小，缺点是资源及时间耗费大一点。
